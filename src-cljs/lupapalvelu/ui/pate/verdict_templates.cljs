...
(rum/defc category-select < rum/reactive
  ...
  [:div.pate-grid-6
   [:div.row
    [:div.col-2.col--full
     (when-let [categories (seq (rum/react state/categories))]
       [:select.dropdown
        (common/add-test-id {:value     (or (rum/react state/current-category) (first categories))
