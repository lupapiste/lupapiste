(ns lupapalvelu.document.canonical-test-common
  (:require [lupapalvelu.document.model :refer [validate-pertinent]]
...
(defn- validate-against-current-schema
  ...
  (validate-pertinent  {:organization {:krysp {:R {:version "2.2.4"}}}
...
(defn validate-all-documents
  ...
  (let [collection (or collection-name :documents)
        ...
        reduced-application (select-keys application [:permitType :auth])]
...
(defn add-ulkomainen-hetu-for-doc [doc]
  (let [path-to-hetu           [:data :henkilo :henkilotiedot]
        ulkomainen-hetu-prefix (some-> doc :schema-info :name (str "-"))]
    (if (and (-> doc :schema-info :type (= "party"))
             (get-in doc path-to-hetu))
      (update-in doc
                 path-to-hetu
                 merge
                 {:not-finnish-hetu        {:modified 1391415675117, :value true}
                  :ulkomainenHenkilotunnus {:modified 1391415675117, :value (str ulkomainen-hetu-prefix "123")}})
      doc)))

(defn add-ulkomainen-hetu-for-docs [app]
  (update app :documents #(map add-ulkomainen-hetu-for-doc %)))
...
(def drawings
  (->> [{:id       1,
        ...
        {:id       4
         ...
         :length   "",
         :height   "333"}]
       (mapv #(assoc % :geometry-wgs84 (draw/wgs84-geometry %)))))
