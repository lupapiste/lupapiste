...
(defn toggle-pate [organizationId flag]
  (fact {:midje/description (format "Toggle Pate for %s: %s"
                                    organizationId flag)}
    (let [permit-type (last (ss/split organizationId #"-"))
          municipality (first (ss/split organizationId #"-"))]
...
(defn add-repeating-setting
  ...
  [apikey organizationId category rep-dict add-dict & kvs]
...
(defn init-verdict-template [apikey organizationId category]
  (command apikey :new-verdict-template
           :organizationId organizationId
           :category (name category)))

(defn set-template-draft-value [apikey organizationId template-id path value]
...
(defn set-template-draft-values [apikey organizationId template-id & args]
  (doseq [[path value] (->arg-map args)]
    (set-template-draft-value apikey organizationId template-id path value)))

(defn publish-verdict-template [apikey organizationId template-id]
...
(defn add-template-condition [apikey organizationId template-id condition]
...
(defn remove-template-condition [apikey organizationId template-id condition-id]
...
(defn check-template-conditions [apikey organizationId template-id & kv]
    ...
    (fact "Verdict PDF attachment has been created"
      ...
      => (contains {:readOnly         true
                    ...
                    :source           {:type "verdicts"
                                       :id   verdict-id}
                    :contents         contents
                    :type             {:type-group (or type-group (if (util/=as-kw permit-type :R)
                                                                    "paatoksenteko"
                                                                    "muut"))
                                       :type-id    "paatos"}
...
(defn- status200-check-proposal [apikey headers {:keys [app-id verdict-id
  ...
  (let [application  (query-application apikey app-id)
    ...
    (fact "Verdict proposal PDF attachment has been created"
      attachment
      => (contains {:readOnly         true
                    :locked           true
                    :source           {:type "verdicts"
