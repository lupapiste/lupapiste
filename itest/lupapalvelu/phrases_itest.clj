...
(fact "No phrases yet"
  (query sipoo :organization-phrases :organizationId sipoo-R-org-id)
  => {:ok true :phrases []})
...
(facts "One phrase listed"
  (let [phrases (:phrases (query sipoo :organization-phrases :organizationId sipoo-R-org-id))]
    ...
    (fact "Update phrase"
      (let [phrase-id (-> phrases first :id)]
        (command sipoo :upsert-phrase
                 ...
                 :phrase "World") => ok?
        (fact "Phrase updated"
          (:phrases (query sipoo :organization-phrases :organizationId sipoo-R-org-id))
          => [{:id       phrase-id
               :tag      "Gat"
               :category "kaava"
               :phrase   "World"}])
...
(fact "Bad org-id"
  (query sipoo :organization-phrases :organizationId "bad")
  ...
  (facts "Phrase categories"
    ...
    (fact "New custom phrase categories"
      (command sipoo :save-phrase-category
               ...
               :organizationId sipoo-R-org-id) => ok?
      (command sipoo :save-phrase-category
