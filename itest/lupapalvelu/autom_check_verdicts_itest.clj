...
(facts "Updating application's verdict attachments"
       ...
       (let [application (create-and-submit-application mikko :propertyId sipoo-property-id :address "Paatoskuja 17")
         ...
         (fact "verdict without attachment is fetched"
               #_(override-krysp-xml sipoo "753-R" :R [{:selector [:yht:liite] :value ""}])
               (command sonja :check-for-verdict :id app-id) => ok?
               #_(remove-krysp-xml-overrides sipoo "753-R" :R)
               )
         ...
         (let [application (query-application sonja app-id)
           ...
           (fact "poytakirja does not have urlHash"
                 poytakirja            => truthy
                 (:urlHash poytakirja) => nil?)

           (fact "foo"
                 (mongo/with-db test-db-name
                   (batchrun/check-for-verdict-attachments)) => nil?))))
