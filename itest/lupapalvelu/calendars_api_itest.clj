...
(facts :ajanvaraus
  (def authority "5056e6d3aa24a1c901e6b9d1")

  (def varaustyypit (atom {}))
  (def authority-calendar-id (atom 0))

  (defn refresh-varaustyypit-atom [tyypit]
    (reset! varaustyypit (zipmap (map (comp keyword :name) tyypit) (map :id tyypit))))
  ...
  (fact "create calendar for existing user"
    ...
    (let [calendars (:calendars (query authority :my-calendars))]
      (count calendars) => 1
      (reset! authority-calendar-id (-> calendars first :id))))
  ...
  (fact "Add reservation type for organization"
    ...
    (let [result (query sipoo :reservation-types-for-organization)
          tyypit (:reservationTypes result)]
      (map :name tyypit) => (just #{"Testityyppi" "Katselmus"})
      (refresh-varaustyypit-atom tyypit)
  ...
  (fact "Delete reservation type created in the previous fact"
    (let [reservation-type-id (get @varaustyypit :Katselmus)]
      ...
      (let [foobar (query sipoo :reservation-types-for-organization)]
        ...
        (refresh-varaustyypit-atom (:reservationTypes foobar)))))
    ...
    (let [tomorrow-at-ten    (t/plus (t/today-at 10 00) (-> 1 t/days))
      ...
      (let [slots (:slots (query authority :calendar-slots
        ...
        (fact "The available slot is deleteable"
              (command authority :delete-calendar-slot
                       :slotId (-> slots first :id)) => ok?
