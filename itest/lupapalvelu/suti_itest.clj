...
(facts "Suti integration administration"
  (fact "Initially empty configuration"
    (query sipoo :suti-admin-details :organizationId sipoo-R-org-id) => (contains {:suti {:server {}}}))
  (fact "No Suti operations"
    (query sipoo :suti-operations :organizationId sipoo-R-org-id) => (contains {:operations nil}))
  (fact "Enable Suti"
    (command sipoo :suti-toggle-enabled :organizationId sipoo-R-org-id :flag true) => ok?)
  (fact "Set bad www address"
    (command sipoo :suti-www :organizationId sipoo-R-org-id :www "bad")
    => (contains {:text "error.invalid.url"}))
  (fact "Set good www addresses"
    (command sipoo :suti-www :organizationId sipoo-R-org-id :www "") => ok?
    (command sipoo :suti-www :organizationId sipoo-R-org-id :www "  http://example.com/$$/suti  ") => ok?)
  (fact "Set bad server details"
    (command sipoo :update-suti-server-details :organizationId sipoo-R-org-id
             :url "bu hao" :username "" :password "")
    => (contains {:text "error.invalid.url"}))
  (fact "Set good server details"
    (command sipoo :update-suti-server-details :organizationId sipoo-R-org-id
             :url "" :username "" :password "") => ok?
    (command sipoo :update-suti-server-details :organizationId sipoo-R-org-id :url "   https://suti.org/   "
             :username "sutiuser" :password "sutipassword") => ok?)
  (fact "Filled configuration"
    (query sipoo :suti-admin-details :organizationId sipoo-R-org-id)
    => {:ok   true
        :suti {:server  {:url "https://suti.org/" :username "sutiuser"}
               :enabled true
               :www     "http://example.com/$$/suti"}})
  (facts "Bad operations"
    (command sipoo :suti-toggle-operation :organizationId sipoo-R-org-id
             :operationId "ronaldos-moth" :flag true)
    => (contains {:text "error.operations.not-found"})
    (command sipoo :suti-toggle-operation :organizationId sipoo-R-org-id
             :operationId "tyonjohtajan-nimeaminen" :flag false)
    => (contains {:text "error.operations.hidden"}))
  (facts "Add good operations"
    (command sipoo :suti-toggle-operation :organizationId sipoo-R-org-id
             :operationId "  kaivuu  " :flag true) => ok?
    (command sipoo :suti-toggle-operation :organizationId sipoo-R-org-id
             :operationId " purkaminen  " :flag true) => ok?
    (query sipoo :suti-operations :organizationId sipoo-R-org-id)
    => (contains {:operations ["kaivuu" "purkaminen"]}))
  (facts "Remove operation"
    (command sipoo :suti-toggle-operation :organizationId sipoo-R-org-id
             :operationId "  kaivuu  " :flag false) => ok?
    (query sipoo :suti-operations :organizationId sipoo-R-org-id
           ) => (contains {:operations ["purkaminen"]})))
...
(facts "Suti and application"
  (let [application-id (create-app-id pena :operation "kerrostalo-rivitalo" :propertyId sipoo-property-id)]
    ...
    (fact "Toggle operation, but disable Suti for the organization"
      (command sipoo :suti-toggle-operation :organizationId sipoo-R-org-id
               :operationId "kerrostalo-rivitalo" :flag true) => ok?
      (command sipoo :update-suti-server-details :organizationId sipoo-R-org-id :url "http://localhost:8000/dev/suti/"
               :username "suti" :password "wrong") => ok?
      (command sipoo :suti-toggle-enabled :organizationId sipoo-R-org-id :flag false) => ok?)
...
(facts "Suti and legacy applications"
  (let [{application-id :id} (create-and-send-application sonja :operation "pientalo" :propertyId sipoo-property-id)
    ...
    (fact "Require Suti for pientalo"
      (command sipoo :suti-toggle-operation :organizationId sipoo-R-org-id :operationId "pientalo" :flag true) => ok?)
