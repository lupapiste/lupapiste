...
(mongo/with-db local-db-name
  ...
  (facts "Organization guest authorities"
    (let [admin (usr/with-org-auth (usr/get-user {:username "sipoo"}))
          ...
          foo "foo@bar.com"]
      (fact "Resolve Veikko"
        (guest/resolve-guest-authority-candidate sipoo-R-org-id veikko)
        => {:firstName "Veikko" :lastName "Viranomainen" :hasAccess false :financialAuthority false})
      (fact "Resolve Luukas"
        (guest/resolve-guest-authority-candidate sipoo-R-org-id "luukas.lukija@sipoo.fi")
        => {:firstName "Luukas" :lastName "Lukija" :hasAccess true :financialAuthority false})
      (fact "Resolve unknown"
        (guest/resolve-guest-authority-candidate sipoo-R-org-id "unknown@example.com")
        => {:hasAccess false :financialAuthority false})
      (fact "No guest authorities"
        (guest/organization-guest-authorities sipoo-R-org-id) => nil)
      (fact "Veikko as a new guest authority"
        (guest/update-guest-authority-organization sipoo-R-org-id admin veikko "Veikko" "Viranomainen" "Dude")
        (guest/organization-guest-authorities sipoo-R-org-id) => [{:name        "Veikko Viranomainen"
                                                                   :email       veikko
                                                                   :description "Dude"}])
      (fact "Organization known guest authorities"
        (guest/known-guest-authority {:data        {:email veikko
                                                    :role  "guestAuthority"}
                                      :application {:organization sipoo-R-org-id}}) => nil
        (guest/known-guest-authority {:data        {:email "not.known@foo.bar"
                                                    :role  "guestAuthority"}
                                      :application {:organization sipoo-R-org-id}}) => itest/fail?)

      (fact "Update Veikko"
        (guest/update-guest-authority-organization sipoo-R-org-id admin veikko "Veikko" "The Man" "Boss")
        (guest/organization-guest-authorities sipoo-R-org-id) => [{:name        "Veikko The Man"
                                                                   :email       veikko
                                                                   :description "Boss"}])
      (fact "Guest authority invite creates new user if needed"
        (usr/get-user-by-email foo) => falsey
        (guest/update-guest-authority-organization sipoo-R-org-id admin foo "Foo" "Bar" "Foo")
        (count (guest/organization-guest-authorities sipoo-R-org-id)) => 2
      ...
      (fact "Remove guest authority"
        (guest/remove-guest-authority-organization sipoo-R-org-id foo)
        (guest/organization-guest-authorities sipoo-R-org-id) => [{:name        "Veikko The Man"
                                                                   :email       veikko
                                                                   :description "Boss"}])))
  ...
  (facts "Application guests and guest authorities"
    (let [app (itest/create-and-submit-local-application itest/pena :property itest/sipoo-property-id
                                                         :address "Gongti beilu 88")
