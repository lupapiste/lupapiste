...
(fact "the ready-calculated validation errors about required document fields, included by a newly created application, are updated when those application fields are filled"
  (let [application-id  (create-app-id pena :propertyId sipoo-property-id)
    ...
    (count errs) => pos?
    (some (fn [{:keys [path result]}]
            (and (= path ["henkilo" "yhteystiedot" "email"])
...
(facts "Application authorities and commenters"
  (let [{j-app-id :id} (create-and-open-application pena
                                                    :operation "aita"
                                                    :propertyId jarvenpaa-property-id)
        {s-app-id :id} (create-and-open-application pena
                                                    :operation "pientalo"
                                                    :propertyId sipoo-property-id)]
    (fact "Järvenpää commenters"
      (:authorities (query raktark-jarvenpaa :application-commenters
                           :id j-app-id))
      => (just {:firstName "Lupasihteeri"
               ...
               {:firstName "Rakennustarkastaja"
                ...
                :roles     ["authority"]}
               :in-any-order))
    (fact "Järvenpää authorities"
      (:authorities (query raktark-jarvenpaa :application-authorities
                           :id j-app-id))
      => (just {:firstName "Lupasihteeri"
                :id        "50ac77ecd2e6c2ea6e73f852"
                :lastName  "Järvenpää"
               ...
               {:firstName "Masa"
                ...
                :roles     ["digitizer"]}
               :in-any-order))
    (fact "Sipoo commenters"
      (:authorities (query sonja :application-commenters
                           :id s-app-id))
      => (just {:firstName "Kosti"
               ...
               {:firstName "Sonja"
                ...
                :roles     ["authority"]}
               :in-any-order))
    (fact "Sipoo authorities"
      (:authorities (query ronja :application-authorities
                           :id s-app-id))
      => (just {:firstName "Ronja"
                :id        "777777777777777777000024"
                :lastName  "Sibbo"
                :roles     ["authority"]}
               {:firstName "Sonja"
...
(facts "Reader, commenter and biller created applications are also opened"
  (doseq [[apikey role] [[luukas "reader"] [kosti "commenter"] [laura "biller"]]]
    (fact {:midje/description role}
      (let [app-id            (create-app-id apikey :propertyId sipoo-property-id)
            {:keys [state opened
                    created]} (query-application apikey app-id)]
        state => "open"
        opened => pos?
        created => pos?
        opened => created))))
...
(state-change-integration sipoo sipoo-R-org-id)
  ...
  (let [application    (create-and-submit-application mikko :municipality sonja-muni)
    ...
    (facts "no vendor backend id (kuntalupatunnus)"
      (fact* "redirects to LP backend url if configured"
        (command sipoo :save-vendor-backend-redirect-config :organizationId sipoo-R-org-id
                 :key :vendorBackendUrlForLpId :val redirect-url) => ok?
      ...
      (fact "error if no LP backend url configured"
        (command sipoo :save-vendor-backend-redirect-config :organizationId sipoo-R-org-id
                 :key :vendorBackendUrlForLpId :val "") => ok?
        (raw sonja :redirect-to-vendor-backend :id application-id) => http404?

        (command sipoo :save-vendor-backend-redirect-config :organizationId sipoo-R-org-id
                 :key :vendorBackendUrlForBackendId :val redirect-url) => ok?
        (raw sonja :redirect-to-vendor-backend :id application-id) => http404?))
      ...
      (let [{verdicts :verdicts} (query-application sonja application-id)
        ...
        (fact* "redirect to backend id url if configured"
          (command sipoo :save-vendor-backend-redirect-config :organizationId sipoo-R-org-id
                   :key :vendorBackendUrlForBackendId :val redirect-url) => ok?
          (command sipoo :save-vendor-backend-redirect-config :organizationId sipoo-R-org-id
                   :key :vendorBackendUrlForLpId :val "http://dontgohere.com") => ok?
          (let [resp (raw sonja :redirect-to-vendor-backend :id application-id) => http303?]
        ...
        (fact* "redirect to LP backend url if available and backend id url not configured"
          (command sipoo :save-vendor-backend-redirect-config :organizationId sipoo-R-org-id
                   :key :vendorBackendUrlForBackendId :val "") => ok?
          (command sipoo :save-vendor-backend-redirect-config :organizationId sipoo-R-org-id
                   :key :vendorBackendUrlForLpId :val redirect-url) => ok?
          (let [resp (raw sonja :redirect-to-vendor-backend :id application-id) => http303?]
            (get-in resp [:headers "location"]) => (str redirect-url application-id)))
...
(fact "Pena cannot create app for organization that has new applications disabled"
  (let [resp  (create-app pena :propertyId "98000000000000")]
    resp =not=> ok?
  ...
  (let [application      (create-and-submit-application mikko :propertyId sipoo-property-id)
    ...
    (fact "there is no suunnittelija"
      ...
      (->> (get-in suunnittelija [:data :henkilotiedot])
           ...
           (into {}))
      => {:etunimi "" :hetu nil :not-finnish-hetu false :sukunimi "" :ulkomainenHenkilotunnus ""})
...
(facts "Set company to document"
  (let [{app-id :id :as app} (create-and-open-application pena :propertyId sipoo-property-id)
    ...
    (fact "No company auth, no company user"
      (->> (command pena :create-doc :id app-id :schemaName "hakija-r")
           :doc
           (company-to-document pena app-id "esimerkki"
                                {:firstName "" :lastName ""
                                 :email "" :phone ""})))
    (fact "No company auth, company user"
      (->> (command erkki :create-doc :id app-id :schemaName "hakija-r")
           :doc
    ...
    (fact "Company auth, no company user"
      (->> (command sonja :create-doc :id app-id :schemaName "hakija-r")
           :doc
  ...
  (fact "Company auth, company user"
    (let [{app-id :id} (create-and-open-application pena :propertyId sipoo-property-id)
          doc-id (->> (command erkki :create-doc :id app-id :schemaName "hakija-r")
                      :doc
                      (company-to-document erkki app-id "esimerkki"
                                           {:firstName "Erkki" :lastName "Esimerkki"
                                            :email "erkki@example.com" :phone "556677"} ))]
...
(facts "Changing application location"
  (let [{application-id :id :as application
         }                (create-application pena
                                             :operation "kerrostalo-rivitalo"
                                             :propertyId sipoo-property-id)
        other-property-id "75341600380014"
        check-address     (fn [address & [property-id]]
                            (let [app (query-application pena application-id)]
                              (fact {:midje/description (str "Address is " address)}
                               (:address app) => address)
                             (when property-id
                               (fact {:midje/description (str "Address is " address)}
                                 (:address app) => address))))]

    (facts "applicant should be able to change location when state is draft"
      (:state application) => "draft"
      ...
      (check-address "Testikatu 7" other-property-id)
      (command pena :change-address :id application-id
               :address "  Retest Road 9  ") => ok?
      (check-address "Retest Road 9"))

    ; applicant submits and authority gives verdict
    (command pena :submit-application :id application-id)
    (command sonja :check-for-verdict :id application-id)

    (facts "applicant should not be authorized to change location anymore"
      (command pena :change-location :id application-id
                                     :x (-> application :location :x) - 1
                                     :y (-> application :location :y) + 1
                                     :address (:address application) :propertyId (:propertyId application) :refreshBuildings false) => fail?
      (command pena :change-address :id application-id :address "Doh") => fail?)

    (fact "authority should still be authorized to change location"
      (command sonja :change-location :id application-id
                                      :x (-> application :location :x) - 1
                                      :y (-> application :location :y) + 1
                                      :address " Auth Ave 12 " :propertyId (:propertyId application) :refreshBuildings false) => ok?
      (check-address "Auth Ave 12" (:propertyId application))
      (command sonja :change-address :id application-id
               :address "Viranomaisen valtatie 5") => ok?
      (check-address "Viranomaisen valtatie 5"))
    (fact "Bad property id"
      (command sonja :change-location :id application-id
               :x (-> application :location :x) - 1
               :y (-> application :location :y) + 1
               :address " Auth Ave 12 " :propertyId "bad property id" :refreshBuildings false)
      => (partial expected-failure? "error.invalid-property-id"))
    (fact "Property id not within the organization"
      (command sonja :change-location :id application-id
               :x (-> application :location :x) - 1
               :y (-> application :location :y) + 1
               :address " Auth Ave 12 " :propertyId jarvenpaa-property-id :refreshBuildings false)
      => (partial expected-failure? "error.property-in-other-municipality"))))
...
(fact "Authority can access drafts, but can't use most important commands"
  (let [id (create-app-id pena)
        ...
        denied-actions #{:delete-attachment :delete-attachment-version :upload-attachment :change-location :change-address
                         ...
                         :toggle-notification-subscription :upsert-application-handler :remove-application-handler
...
(facts "Changing application state to extinct"
  (let [application-id   (create-app-id pena :operation "kerrostalo-rivitalo" :propertyId sipoo-property-id)
        get-attachments  #(:attachments (query-application sonja application-id))
        metadata-before  {:myyntipalvelu   true
                          :nakyvyys        "julkinen"
                          ...
                          :sailytysaika    {:arkistointi "ikuisesti"
                                            :perustelu   "Syyt §123"}}
        extinct-defaults {:myyntipalvelu false
                          :nakyvyys      "viranomainen"}
        metadata-after   (merge metadata-before extinct-defaults)
        metadata-valid?  (fn [{:keys [metadata] :as att}]
                           (every? (fn [[k v]]
                                     (= v (k metadata)))
                                   metadata-after))]
    ;; Applicant submits and authority gives verdict and metadata
    (command pena :submit-application :id application-id)
    (command sonja :check-for-verdict :id application-id)
    (command sonja :store-tos-metadata-for-application
             :id application-id
             :metadata metadata-before)
    => ok?
    (doseq [{:keys [id]} (get-attachments)]
      (command sonja :store-tos-metadata-for-attachment
               :id application-id
               :attachmentId id
               :metadata metadata-before)
      => ok?)

    (fact "Sonja changes application state to extinct"
      (command sonja :change-application-state :id application-id :state "extinct") => ok?
      (:state (query-application sonja application-id)) => "extinct")
    (fact "Application metadata updated to extinct defaults"
      (metadata-valid? (query-application sonja application-id)) => true)
    (fact "Application attachment metadata updated to extinct defaults"
      (every? metadata-valid? (get-attachments)) => true)))
...
(facts "Returning application to draft state after submission"
  (fact "Only applications in 'submitted' state can be returned to draft"
    (let [application-id (create-app-id pena :operation "kerrostalo-rivitalo" :propertyId sipoo-property-id)]
      ...
      (command sonja :check-for-verdict :id application-id)
      (return-to-draft sonja application-id) => (partial expected-failure? :error.command-illegal-state)))
  (fact "The writers are notified of the return to draft"
    (let [application-id (create-app-id pena :operation "kerrostalo-rivitalo" :propertyId sipoo-property-id)
          application    (query-application pena application-id)
          hakija         (domain/get-applicant-document (:documents application))]
      ...
      (let [emails (sent-emails)]
        (fact "Mikko and Erkki (Pena's company admin) get email"
          (count emails) => 2
          emails => (partial every? (comp (partial re-find #"hakemuksen Asuinkerrostalon tai rivitalon rakentaminen osoitteessa") :html :body))
          ...
          emails => (partial some (comp (partial re-find (re-pattern (email-for-key erkki))) :to))))))
...
(fact "Remove handlers from reverted draft flag"
  (let [application-id (create-app-id teppo :operation "kerrostalo-rivitalo" :propertyId sipoo-property-id)]
    ...
    (fact "Set the remove handlers from reverted draft flag"
      (command sipoo :set-organization-remove-handlers-from-reverted-draft :organizationId "753-R"
               :enabled true) => ok?)
    (return-to-draft sonja application-id)
    (let [application (query-application teppo application-id)]
