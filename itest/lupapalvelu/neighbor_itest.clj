...
(defn get-neighbors [apikey app-id]
  (:neighbors (query-application apikey app-id)))
...
(facts "create app, update neighbor"
  (let [[application neighborId] (create-app-with-neighbor)
        ...
        _                        (command sonja :neighbor-update :id application-id
                                          :neighborId neighborId
                                          :propertyId "12312312341200"
                                          :name "n2" :street "s2"
                                          :city "c2" :zip "z2" :email "e2")
        neighbors                (get-neighbors sonja application-id)
        neighbor                 (util/find-by-id neighborId neighbors)]
    (fact "Neighbor must exist"
      (command sonja :neighbor-update :id application-id
               :neighborId "bad-id"
               :propertyId "12312312341200"
               :name "n2" :street "s2"
               :city "c2" :zip "z2" :email "e2")
      => (err :error.neighbor-not-found))
...
(facts "create app, remove neighbor"
  (let [[application neighborId] (create-app-with-neighbor)
        application-id (:id application)]
    (fact "Neighbor must exist"
      (command sonja :neighbor-remove :id application-id
               :neighborId "bad-id")
      => (err :error.neighbor-not-found))
    (fact "Existing neighbor"
      (command sonja :neighbor-remove :id application-id :neighborId neighborId)
      (get-neighbors sonja application-id) => empty?)))
...
(facts "neighbor invite email has..."
  (let [neighbor-email-addr       "abba@example.com"
        ...
        expiration-date           (date/finnish-date (+ ttl/neighbor-token-ttl (now)) :zero-pad)]
  ...
  (let [[{application-id :id :as application}
    ...
    (let [resp        (query pena :neighbor-application
        ...
        (let [document-types (set (map (comp :name :schema-info) (:documents application)))
          ...
          (fact "does not have maksaja document" "maksaja" =not=> has-doc))
    ...
    (let [nid (name neighborId)]
      (facts "Response in the application"
        (let [{:keys [neighbors attachments]} (query-application sonja application-id)]
          (-> (util/find-by-id nid neighbors) :status last)
          => (contains {:message "kehno suunta"
                        :state   "response-given-comments"})
          (count attachments) => 6
          (some (util/fn-> :source :id (= nid)) attachments) => true
          (last attachments) => (contains {:source {:id nid :type "neighbors"}
                                           :type   {:type-group "ennakkoluvat_ja_lausunnot"
                                                    :type-id    "naapurin_huomautus"}})))

      (fact "Remove neighbor after response"
        (command sonja :neighbor-remove :id application-id
                 :neighborId nid) => ok?)
      (fact "Neighbor and response attachment are gone"
        (let [{:keys [neighbors attachments]} (query-application sonja application-id)]
          (count attachments) => 5
          (util/find-by-id nid neighbors) => nil
          (not-any? (util/fn-> :source :id (= nid)) attachments) => true))))))
