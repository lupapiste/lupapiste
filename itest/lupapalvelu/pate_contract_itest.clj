...
(def ^:private sipoo-YA-org-id "753-YA")
(def ^:private sipoo-YA-org-id "753-YA")
...
(facts "Modern contracts"
  (toggle-pate sipoo-YA-org-id true)
    ...
    (let [{app-id :id} (create-and-submit-application teppo
      ...
      (facts "Contract settings consist only of organization name"
          (query sipoo-ya :verdict-template-settings
                 :organizationId sipoo-YA-org-id
                 :category "contract")
          => {:ok       true
              :filled   false
              :settings nil}
          ...
          (fact "The settings are now filled"
            (query sipoo-ya :verdict-template-settings :organizationId sipoo-YA-org-id :category "contract")
            => (contains {:filled true}))
          (fact "Create contract template"
            (let [{template-id :id} (init-verdict-template sipoo-ya sipoo-YA-org-id :contract)]
              (fact "Fill template"
              ...
              (fact "Add condition"
                (add-template-condition sipoo-ya sipoo-YA-org-id template-id
                                        "Some random condition."))
              (fact "Publish template"
                (publish-verdict-template sipoo-ya sipoo-YA-org-id template-id) => ok?)
              (fact "New contract"
                ...
                (fact "Signature request can be sent"
                  ...
                  (let [{:keys [parties ok]} (query sonja :signature-request-parties :id app-id
                                                    :verdict-id verdict-id)
                        signer-id            (->> parties first :value)
                        signature-request    (fn [target-id]
                                            (command sonja :send-signature-request :id app-id
                                                     :verdict-id verdict-id
                                                     :signer-id target-id))]
                    ...
                    (fact "Signature request can be sent"
                      (signature-request signer-id) => ok?)

                    (fact "Contract has signature request"
                      (let [{:keys [pate-verdicts]} (query-application sonja app-id)]
                        (->> pate-verdicts first :signature-requests first)
                        => (contains {:name "Teppo Nieminen"})))

                    (fact "Request cannot be resent"
                      (signature-request signer-id)
                      => (err :error.bad-signature-request))

                    (fact "Request target id must be valid"
                      (signature-request "random")
                      => (err :error.bad-signature-request))))))))))))
