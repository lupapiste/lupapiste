...
(fact "Organization links"
      (let [app-id (create-app-id pena :propertyId sipoo-property-id :operation "pientalo")
        ...
        (fact "Update the first link"
              (command sipoo :update-organization-link :organizationId sipoo-R-org-id
                       :url example-url
                       :name example-name
                       :index 0)
              => ok?)
        (fact "Now the first link is new again"
              (-> (query pena :organization-links :id app-id :lang "fi")
                  :links first) => {:url (ss/trim (:fi example-url)) :text "Esimerkki" :isNew true})
        (fact "Add new link"
              (command sipoo :add-organization-link :organizationId sipoo-R-org-id
                       :url lupapiste-url
                       :name lupapiste-name) => ok?)
        ...
        (facts "Organization link urls must be valid"
               ...
               (fact "Update link"
                     ...
                     => {:ok false, :text "error.invalid.url"})
               (fact "Remove link"
                     (command sipoo :remove-organization-link :organizationId sipoo-R-org-id
                              :url (assoc lupapiste-url
                                          :fi "not-a-valid-url")
                              :name lupapiste-name)
                     => {:ok false, :text "error.invalid.url"})))
