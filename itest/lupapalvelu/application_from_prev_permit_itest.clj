...
;    (println "\n")
    (println "\n example-app-info: ")
    (clojure.pprint/pprint example-app-info)
    (println "\n")
...
;
;      (against-background
;        (mongo/by-id :applications anything) => {:id "valid-lupapiste-id"} :times 1))

    (against-background
      (krysp-reader/get-app-info-from-message anything anything) => (assoc example-app-info :id "valid-lupapiste-id")))
