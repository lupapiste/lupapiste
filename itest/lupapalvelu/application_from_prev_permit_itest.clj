...
#_(defn- create-app-from-prev-permit [apikey & args]
   (let [args (->> args
                (apply hash-map)
...
#_(facts "Creating new application based on a prev permit"
   ...
   (let [example-xml (xml/parse (slurp (io/resource "../resources/krysp/sample/verdict-rakval-from-kuntalupatunnus-query.xml")))
     ...
     ; 7: testaa tilanne kun do-create-application heittaa jonkin poikkeuksen -> (fail :error.no-previous-permit-found-from-backend)
