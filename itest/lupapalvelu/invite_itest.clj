...
(facts* "Applicant invites designer"
  ...
  (let [application-id (create-app-id mikko :propertyId sipoo-property-id :address "Kutsukatu 13")
    ...
    (facts "email is validated"
      ...
      (fact "Email CAN contain non-ascii chars"
        (invite mikko application-id suunnittelija-doc "suunnittelija" "eero.er\u00e4tuli@example.com") => ok?))
    ...
    (fact "Can't invite app's organization's authority"
      (invite mikko application-id nil nil (email-for-key sonja) "" "")
      => {:ok false :text "invite.invited-org-authority"})

    (fact "Can invite other organization's authority"
      (invite mikko application-id nil nil (email-for-key raktark-jarvenpaa) "" "") => ok?)
    ...
    (fact "Mikko can't unsubscribe Teppo's notifications"
      (command pena :toggle-notification-subscription :id application-id :authId teppo-id :subscribe false)
      => not-accessible?)
    ...
    (fact "Mikko prefils Teppo's company name"
      (command mikko :update-doc :id application-id :doc suunnittelija-doc :updates [["yritys.yritysnimi" "Tepon saneeraus"]]) => ok?)
