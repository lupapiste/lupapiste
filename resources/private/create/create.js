;(function() {
  ...
  function CreateApplicationModel() {
    ...
    self.operationTree = ko.observable();
    self.lastPageParams = self;
    self.lastPageInitFn = function() {
      self.updateOrganizationDetails( self.operation() );
    };

    hub.subscribe("create-step-2", function() {
      pageutil.openPage("create-part-2");
      self.operationTree(_.map(self.operations(), operations2tree));
      window.scrollTo(0, 0);
    });
    ...
    self.operationInfos = ko.observable({});
    ...
    self.infoRequestLocKeyPostfix = ko.computed(function() {
      var permitType = _.get(self.operationInfos(), [self.operation(), "permit-type"]);
      return _.some(["YL", "YI", "YM", "MAL", "VVVL"], permitType) ? ".ymp" : "";
    });
    ...
    self.findOperations = function(code) {
      municipalities.operationsForMunicipality(code, function(operations, opInfos) {
        self.operations(operations);
        self.operationInfos(opInfos);
      });
      return self;
    };
