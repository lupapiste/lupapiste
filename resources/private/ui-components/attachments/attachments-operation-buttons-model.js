...
LUPAPISTE.AttachmentsOperationButtonsModel = function(params) {
  ...
  self.updateMultipleAttachments = function() {
    hub.send("show-dialog", {ltitle: "update-attachments.title",
                             size: "medium",
                             component: "multi-attachment-edit-dialog",
                             componentParams: {lyesTitle: "update-attachments.choose-files",
                                               lnoTitle: "update-attachments.cancel",
                                               noFn: _.partial(hub.send, "close-dialog")
    }});
  };

  self.canEdit = function() {
    return (self.authModel.ok("bind-attachments") && self.hasFiles());
  };
  ...
  self.canExport = function() {
    return self.authModel.ok( "move-attachments-to-backing-system" )
      || self.authModel.ok( "attachments-to-asianhallinta" );
  };
