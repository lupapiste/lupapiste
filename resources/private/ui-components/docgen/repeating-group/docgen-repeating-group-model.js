LUPAPISTE.DocgenRepeatingGroupModel = function(params) {
  ...
  self.groupsRemovable = function(schema, ignoreReadOnly) {
    return (ignoreReadOnly || !_.some(schema.body, "readonly")) &&
      !params.isDisabled &&
  ...
  self.addGroupWith = function(data) {
      self.service.addRepeatingGroupWithData(params.documentId, params.path, self.indicator, data);
  };
  ...
  self.duplicateLastGroupWith = function(extraData) {
    var sourceIndex = _(self.groups()).map("index").map(_.parseInt).max();
    self.service.copyRepeatingGroupWithExtraData(params.documentId, params.path, sourceIndex, self.indicator, extraData);
  };
  ...
  self.allowRepeatingEmpty = function() {
      var repeatingModel = self.service.getInDocument(params.documentId, params.path);
      return repeatingModel.schema["repeating-allow-empty"];
  };

  if (!self.allowRepeatingEmpty()) {
