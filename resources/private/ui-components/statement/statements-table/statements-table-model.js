LUPAPISTE.StatementsTableModel = function(params) {
  ...
  self.statementIdsWithAttachments = self.disposedPureComputed(function() {
  ...
  self.isRemovable = function(statement) {
    return self.authorization.ok( "delete-statement")
      && !self.isExternal(statement)
      && !self.isGiven(statement);
  };

  self.showDeleteColumn = self.disposedPureComputed( function() {
    return _.some( self.statements(), self.isRemovable );
  });
  ...
  self.repliesEnabled = self.disposedPureComputed(function() {
