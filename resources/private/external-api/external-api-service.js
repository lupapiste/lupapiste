...
LUPAPISTE.ExternalApiService = function() {
  ...
  */
  ...
  // Convenience hubscription for cljs client code.
  hub.subscribe( "external-api::attachments-sent", function() {
    var application = lupapisteApp.models.application;
    if( application
        && lupapisteApp.models.applicationAuthModel.ok( "move-attachments-to-backing-system")
        && application.externalApi.enabled() ) {
      hub.send( "external-api::integration-sent",
                externalApiTools.toExternalPermit( application._js ));
    }
  });
