LUPAPISTE.DocumentDataService = function(params) {
  ...
  self.findDocumentById = function(id) {
    return _.find(self.model.peek(), {id: id});
  };

  self.findDocumentByName = function( name ) {
    return _.find( self.model.peek(), {name: name});
  };
  ...
  self.addRepeatingGroup = function(documentId, path) {
    ...
    pushToRepeating(repeatingModel, {});
    return _.last( repeatingModel.model()).index;
  };

  self.addRepeatingGroupWithData = function(documentId, path, indicator, data) {
    var repeatingModel = self.getInDocument(documentId, path);
    var repLength = pushToRepeating(repeatingModel, data);
  ...
  self.copyRepeatingGroupWithExtraData = function(documentId, path, index, indicator, extraData) {
    var repeatingModel = self.getInDocument(documentId, path);
    var rawModel = getAsRaw(findByIndex(repeatingModel, index));
    var rawModelWithExtraData = _.merge(rawModel, extraData);
    var repLength = pushToRepeating(repeatingModel, rawModelWithExtraData);
    var updates = getAsUpdates(repeatingModel.model()[repLength - 1]);
    self.updateDoc(documentId,
                   updates,
                   indicator);
  ...
  function command(commandName, collection, documentId, params, opts) {
    ...
    ajax
      ...
      .success(function(e) {
        ...
        hub.send( commandName + "-success", {
          paths: params.updates || [[(_.join( params.path, "." ))]],
  ...
  // Returns the whitelisted status (disabled or hidden) if according to the whitelist.
  function listStatus( schema ) {
    var status = util.getIn(schema, ["whitelist", "otherwise"]);
    if (status && !_.includes(util.getIn(schema, ["whitelist", "roles"]),
                              lupapisteApp.models.currentUser.applicationRole())) {
      return status;
    }
  }

  self.isListDisabled = function( schema ) {
    return listStatus( schema ) === "disabled";
  };

  self.isListHidden = function( schema ) {
    return listStatus( schema ) === "hidden";
  };
