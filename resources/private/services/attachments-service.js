...
LUPAPISTE.AttachmentsService = function() {
  "use strict";
  var self = this,
      filters = ko.mapping.fromJS({
        hakemuksenAikaiset: false,
        rakentamisenAikaiset: false,
        ivSuunnitelmat: false,
        kvvSuunnitelmat: false,
        rakennesuunnitelmat: false,
        liiteEiTarpeen: false,
        paapiirrustukset: false
      }),
      dummyData = [
        {
          "type" : {
  ...
  function createAttachmentModel(attachment) {
    return ko.observable({
      ...
      type:      _.get(attachment, "type"),
      file:      ko.observable(_.pick(attachment.latestVersion,
                                      ["filename", "contentType", "user"])),
      state:     ko.observable(_.get(attachment, "state")),
      notNeeded: ko.observable(_.get(attachment, "notNeeded"))
    });
  }
  ...
  self.approveAttachment = function(attachmentId) {
    self.getAttachment(attachmentId).peek().state("ok");
  };
  self.rejectAttachment = function(attachmentId) {
    self.getAttachment(attachmentId).peek().state("requires_user_action");
  };
  self.isApproved = function(attachmentId) {
    return self.getAttachment(attachmentId)().state() === "ok";
  };
  self.isRejected = function(attachmentId) {
    return self.getAttachment(attachmentId)().state() === "requires_user_action";
  };
  ...
  function getAttachmentsByTypes() {
    return _(self.attachments())
      .map(function(attachment) {
        return _.pick(attachment.peek(), ["type", "id"]);
      })
      .groupBy(function(attachment) {
        return attachment.type["type-group"];
      })
      .mapValues(function(attachmentsOfTypeGroup) {
        return _(attachmentsOfTypeGroup)
          .groupBy(function(attachment) {
  ...
  self.attachmentsByTypes = ko.computed(getAttachmentsByTypes);
};
