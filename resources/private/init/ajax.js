// TLDR;
// USE js/lpAjax to refer this class in ClojureScript.
// DO **NOT** USE js/ajax in ClojureScript
// In Javascript both ajax and lpAjax works.
//
// Longer version:
// Lupapiste ajax is stored to two variables because when :advanced build is used cljs-ajax namespace combined with
// the way externs are generated conflicts with global ajax varible. Effectively, this class can be called in
// clojure script simply  with `js/ajax`. However, it is possible to call this from ClojureScript by refering this
// with `js/lpAjax`. Global ajax variable does not cause any problems as long as it is not refered in ClojureScript,
// because then :advanced complication does not generate a symbol that conflicts with cljs-ajax root namespace.

var ajax, lpAjax;
ajax = lpAjax = (function($) {
  "use strict";

  var callId = 0;
  var preprocessors = [];

  function pushPreprocessor(preprocess) { preprocessors.push(preprocess); }

  function applyPreprocessors(params) {
    return preprocessors.reduceRight(function (params, preprocess) { return preprocess(params); }, params);
  }
  ...
  function Call(url, type) {
    ...
    self.onTimeout = function(f) {
      self.timeoutHandler = f;
      return self;
    };
  ...
  function command(name, params) {
    return new Call("/api/command/" + name, "POST").json(applyPreprocessors(params));
  }

  function query(name, params) {
    return new Call("/api/query/" + name, "GET").params(applyPreprocessors(params));
  }

  function datatables(name, params) {
    return new Call("/api/datatables/" + name, "POST").json(applyPreprocessors(params));
  }
  ...
  return {
    pushPreprocessor: pushPreprocessor,
    post:      post,
    postJson:  postJson,
    get:       get,
    deleteReq: deleteReq,
