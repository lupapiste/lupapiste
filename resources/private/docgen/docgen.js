...
console.log(v);
  ...
  function makeDocModel(spec, model, callback, docId, appId) {
    return new DocModel(spec, model, callback, docId, appId);
  }

  return {
    build: function(spec, model, callback, docId, appId) {
      return new DocModel(spec, model, callback, docId, appId);
    },
    createDocumentButton: function(id, label) {
      var appendButton = document.createElement("button");
      appendButton.id = id;
      appendButton.className = "btn";
      appendButton.innerHTML = label;
