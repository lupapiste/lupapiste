...
Pena uploads an attachment for which there is an automatic assignment trigger in Sipoo
...
Sonja opens attachments tab and sees 'ELY ja naapuri' filter
  ...
  Wait until  Total attachments row count is  1
  Element should be visible  jquery=tr[data-test-type='paapiirustus.asemapiirros']
  Element should not be visible  jquery=tr[data-test-type='ennakkoluvat_ja_lausunnot.elyn_tai_kunnan_poikkeamapaatos']
  Toggle toggle  assignment-dead1111111111111112beef-filter
  Scroll and click test id  filter-link-dead1111111111111111beef
  Wait until  Total attachments row count is  1
  Element should not be visible  jquery=tr[data-test-type='paapiirustus.asemapiirros']
