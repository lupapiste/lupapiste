...
Admin admin logs in and opens Sipoo-R
  SolitaAdmin logs in
  Go to  ${SERVER}/app/fi/admin#!/organization/753-R
  Scroll to test id  change-handler

Sonja (Muutettu) to Laura
  Select from autocomplete by test id  old-handler  Sonja
  Select from autocomplete by test id  new-handler  Laura
  Select from list by test id and label  handler-role  Muutettu

No changes if states do not match
  Toggle toggle  archived
  Toggle toggle  final
  Toggle toggle  extinct
  Click enabled by test id  change-handler
  Test id text is  change-result  Ei päivitettyjä hakemuksia.

Add sent state and try again
  Toggle toggle  sent
  Click enabled by test id  change-handler
  Test id text is  change-result  Yksi päivitetty hakemus.

New and old handlers cannot be the same
  Select from autocomplete by test id  new-handler  Sonja
  Test id disabled  change-handler
  [Teardown]  Logout
...
Sonja logs in and checks the bulk change result
  Sonja logs in
  Open application  Duplicate fix  ${propertyId}
  List handler is  0  Laskuttaja Laura  Muutettu

Inforequest handler is unchanged
  Go to page  applications
  Wait until  Open inforequest  ${appname}-info  ${propertyId}
  List handler is  0  Sibbo Sonja  Muutettu

Other application handlers are unchanged
  Go to page  applications
  Wait until  Open application  ${appname}  ${propertyId}
  List handler is  0  Sibbo Ronja  Muutettu
  List handler is  1  Sibbo Sonja  Uusi
  [Teardown]  Logout
