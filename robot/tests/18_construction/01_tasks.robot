...
# Vastaava IV-työnjohtaja is under requirements
Rakentaminen tab contains 2 foreman tasks
  Wait until  jQuery Should Match X Times  table.all-foremen-table tbody tr  2
...
# Vastaava IV-työnjohtaja is under requirements
Rakentaminen tab contains 4 requirements (lupamaarays)
  Wait until  Xpath Should Match X Times  //div[@id='application-tasks-tab']//table[@data-test-id='lupamaaraykset']//tbody/tr  4

Katselmukset
  Wait Until  Page should contain  Kokoukset, katselmukset ja tarkastukset
  Task count is  task-katselmus  3

None of the reviews is done
  Review row is not done  0
  Review row is not done  1
  Review row is not done  2
...
Muut lupamaaraykset
  ...
  Task state count is  task-lupamaarays  requires_user_action  4
  [Teardown]  Logout
...
Making the latest Aloituskokous final will not finalize others
  Open review  2
