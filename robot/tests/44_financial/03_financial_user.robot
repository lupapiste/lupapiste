...
Tasks tab is visible but everything is disabled
  Open tab  tasks
  Wait until  Element should be disabled  invite-other-foreman-button
...
Attachments tab is visible and attachments can be downloaded
  ...
  Test id enabled  download-accordion-attachments-button-undefined
  Test id enabled  download-all-attachments-button
  Test id enabled  download-all
...
Financial authority can go to my page and change email
  ...
  Go to  ${SERVER}/app/fi/financial-authority
  Go to page  mypage
  Input text with jQuery  \#newEmail  financial@ara.com
