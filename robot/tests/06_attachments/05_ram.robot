...
Check link row
  ...
  Wait until element contains  jquery=td[data-test-id=ram-link-type-${index}]  ${type}
  Wait until element contains  jquery=td[data-test-id=ram-link-file-${index}]  ${filename}
  Wait until element contains  jquery=td[data-test-id=ram-link-editor-${index}]  ${editor}
  Wait until element contains  jquery=td[data-test-id=ram-link-approver-${index}]  ${approver}  
  
Delete disallowed
  [Arguments]  ${click}=False
  Run keyword if  ${click}  Click button  id=show-attachment-versions
