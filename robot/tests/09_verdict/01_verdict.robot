...
Sonja fetches verdict from municipality KuntaGML service
  Open tab  verdict
  Fetch verdict
  Check verdict row  0  §1 myönnetty  1.9.2013  Puupaazz elmo viranomainen
...
Add foreman
  Add legacy foreman  0  erityisalojen työnjohtaja

Add condition
  Add legacy condition  0  Bajamajoja oltava riittvästi
  # Debounce before we leave page
  Sleep  1.2s
...
Add and delete verdict draft
  Open tab  verdict
  Verdict count is  4  #TODO FIX THIS. VERDICT COUNT IS ACTUAllY 3 BUT BUTTON 'TOIMITA PÄÄTÖS' ADDS EXTRA TABLE VALUE
  Go to give new legacy verdict
  Click back
  Verdict count is  5  #TODO SAME AS ABOVE
  Delete verdict  0
  Verdict count is  4  #TODO SAME AS ABOVE
...
Stamping page opens, backing system verdict details can be seen
  ...
  Textfield value should be  xpath=//div[@id="stamping-container"]//form[@id="stamp-info"]//input[@data-test-id="stamp-info-kuntalupatunnus"]  123567890
  Should not be empty  xpath=//div[@id="stamping-container"]//form[@id="stamp-info"]//input[@data-test-id="stamp-info-section"]
  Textfield should contain  xpath=//div[@id="stamping-container"]//form[@id="stamp-info"]//input[@data-test-id="stamp-info-section"]  § 22
  [Teardown]  Logout
