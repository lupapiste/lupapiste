...
Foreman can nowadays, after LPK-5142, invite persons to application
  Open tab  parties
  Wait test id visible  application-guest-add
  Wait test id visible  application-invite-person
...
Foreman could comment on project application LPK-5142
  Open project application
  Confirm yes no dialog
  Open side panel  conversation
  Element should be visible by test id  application-new-comment-text
  Element should be visible by test id  application-new-comment-btn
...
Link foreman approval to base app
  ...
  Open tab  tasks
  Required foreman state is  Vastaava ty\u00f6njohtaja  required

  # FIXME app link selection is not available anymore
  # test cases do not work from here on, needs rewrite
...
Foreman state has reset on base app
  [Tags]  fail
  # FIXME foreman naming process has changed
  Go back to project application
  Wait Until  Application state should be  verdictGiven
  Open tab  tasks
