...
Sonja invites foreman to application
  ...
  Wait until  Primary operation is  tyonjohtajan-nimeaminen-v2
...
Add työnjohtaja task to current application
  [Arguments]  ${name}  ${role}=työnjohtaja
  Open tab  tasks
  ...
  Select From List By Value  choose-task-subtype   ${role}
  Input text  create-task-name  ${name}
...
Required foreman state is
  ...
  ${s} =  Get Element Attribute  xpath=//table[contains(@class, 'all-foremen-table')]//tr[@data-test-name="${role}"]  data-foreman-state
