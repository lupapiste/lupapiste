...
Library        CustomSeleniumLibrary.py  timeout=10  run_on_failure=Nothing
Library        String
Library        Collections
...
${SET TIMESTAMP URL}            ${SERVER}/dev/set-application-timestamp
...
# Our Wait Until does not handle kw-args properly, so this extra
# keyword is needed for Xpath/jQuery Should Match X Times
Locator should match X times
  [Arguments]  ${locator}  ${times}
  Page Should Contain Element  ${locator}  limit=${times}

Xpath Should Match X Times
  [Arguments]  ${xpath}  ${times}
  Run keyword unless  ${times}==0  Wait until  Locator should match X times  xpath:${xpath}  ${times}
...
jQuery Should Match X Times
  ...
  Run keyword unless  ${times}==0  Wait until  Locator should match X times  jquery:${selector}  ${times}
...
Open browser to login page
  ...
  Set Selenium Timeout  ${WAIT_DELAY}
...
Wait for jQuery
  Wait For Condition  return (typeof jQuery !== "undefined") && jQuery.active===0;  7
...
Open tab
  ...
  Scroll to top
  Sleep  0.1s
  Run keyword unless  ${is-visible}  Click by test id  application-open-${name}-tab
...
Laura logs in
  [Arguments]  ${showAll}=True
  Authority logs in  laura  laura  Laura Laskuttaja  ${showAll}
...
Select From Autocomplete
  ...
  Press keys  jquery=${container} input[data-test-id="autocomplete-input"]  RETURN
...
Select from cljs-autocomplete by test id
  [Arguments]  ${data-test-id}  ${value}
  ${selector}=  Set Variable  //div[@data-test-id="${data-test-id}" and contains(@class, 'pate-autocomplete')]
  Wait until  Element should be visible  xpath=${selector}
  Click element  xpath=${selector}
  Click element  xpath=${selector}//ul/li[text()[contains(.,"${value}")]]


Clear rum text field
  [Arguments]  ${selector}
  # Have to use backspaces because React does not play nicely with "Input text" or "Clear element"
  ...
  ...  Repeat Keyword  ${backspaces count +1}  Press Key  ${selector}   \\08

Clear rum text field by test id
  [Arguments]  ${data-test-id}
  Clear rum text field  xpath=//input[@data-test-id='${data-test-id}']
...
Autocomplete selection by test id is
  [Arguments]  ${tid}  ${value}
  Element should contain  jquery=[data-test-id=${tid}] span.autocomplete-selection span.caption  ${value}

Autocomplete selection by test id contains
  [Arguments]  ${tid}  ${value}
  Element should contain  jquery=[data-test-id=${tid}] span.autocomplete-selection  ${value}

Autocomplete selection by test id is empty
  [Arguments]  ${tid}
  Element should not be visible  jquery=[data-test-id=${tid}] span.autocomplete-selection span.caption
  Element should not be visible  jquery=[data-test-id=${tid}] span.autocomplete-selection ul.tags .tag
...
Element should be visible by test id
  [Arguments]  ${id}
  Element Should Be Visible  xpath=//*[@data-test-id="${id}"]

Element should not be visible by test id
  [Arguments]  ${id}
  Element Should Not Be Visible  xpath=//*[@data-test-id="${id}"]
...
Set application timestamp
  [Arguments]  ${address}  ${attr}  ${val}
  Go to  ${SET TIMESTAMP URL}?address=${address}&attr=${attr}&val=${val}&redirect=true
...
Create application the fast way
  ...
  Test id text is  application-property-id  ${propertyId}
  Kill dev-box
...
Create inforequest the fast way
  ...
  Test id text is  inforequest-property-id  ${propertyId}
  Kill dev-box
...
Upload batch file
  [Arguments]  ${index}  ${path}  ${type}  ${contents}  ${grouping}=${EMPTY}  ${testId}=add-attachments-input
...
Open the request
  ...
  Go to page  applications
  Wait test id visible  search-tab-all
...
Foreman count is
  [Arguments]  ${amount}
  Wait until  Xpath Should Match X Times  //table[contains(@class, 'all-foremen-table')]/tbody/tr  ${amount}
...
Neighbor application address should be
  [Arguments]  ${address}
  Wait Until  Element Should Be Visible  xpath=//section[@id='neighbor']//span[@data-test-id='application-title']
  Wait Until  Element text should be  xpath=//section[@id='neighbor']//span[@data-test-id='application-title']  ${address}
...
Scroll and click
  [Arguments]  ${selector}
  Scroll to  ${selector}
  Wait until  Click Element  jquery=${selector}
...
Test id should contain
  [Arguments]  ${id}  ${text}
  Wait Until Element Contains  //*[@data-test-id="${id}"]  ${text}
...
Checkbox wrapper disabled
  [Arguments]  ${id}
  Element should be disabled  xpath=//input[@id="${id}"]
...
Press key test id
  [Arguments]  ${tid}  ${key}
  Press key  //*[@data-test-id="${tid}"]  ${key}

Navigate from sidebar to
  [Arguments]  ${page}
  ${menu}=  Run Keyword and Return Status  Wait test id visible  sidebar-menu
...
Print frontend error texts
  ...
  FOR  ${idx}  IN RANGE    1    10
    ${ELEM_COUNT}=  Get Matching Xpath Count  ${ROW_XPATH}\[${idx}]
    Exit for loop if  ${ELEM_COUNT} == 0
    ${VAL}=  Get Text  ${ROW_XPATH}\[${idx}]
...
Test id required
  [Arguments]  ${tid}
  ${tid}=  Quote  ${tid}
  Wait until element is visible  jquery:[data-test-id=${tid}][required][aria-required=true]

Test id not required
  [Arguments]  ${tid}
  Wait test id visible  ${tid}
  ${tid}=  Quote  ${tid}
  Wait until element is not visible  jquery:[data-test-id=${tid}][required][aria-required=true]

Test id invalid
  [Arguments]  ${tid}
  ${tid}=  Quote  ${tid}
  Wait until element is visible  jquery:[data-test-id=${tid}][aria-invalid=true]

Test id not invalid
  [Arguments]  ${tid}
  Wait test id visible  ${tid}
  ${tid}=  Quote  ${tid}
  Wait until element is not visible  jquery:[data-test-id=${tid}][aria-invalid=true]


# OK = Not required or invalid
Test id OK
  [Arguments]  ${tid}
  Test id not required  ${tid}
  Test id not invalid  ${tid}
