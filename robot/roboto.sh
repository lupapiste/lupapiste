...
start_lupapiste() {
   ...
   lein do front:prod, run &> robot/target/lupapiste.log &
   LUPISPID=$!
...
check_env() {
   ...
   echo "$SELENIUM" | grep -E "selenium\s+\(3\.1[3-9][0-9]?\.[0-9]+\)" || versionfail "Your selenium version '$SELENIUM' may cause tests to fail. Update ${BASH_SOURCE}:${LINENO} to match the current selenium version if it is fine, or change to 3.X.X."
   ...
   case "$BROWSER" in
      ...
      "chrome" | "headlesschrome" )
         ...
         echo "$CG" | grep -q "^Google Chrome 88\." || versionfail "Major version '$CG' of Chrome may not work yet. Update ${BASH_SOURCE}:${LINENO} if this is fine."
...
run_test() {
   ...
   echo "NOTE: robot exited with $BOT after round $ROUND/$LASTROUND" >> target/$TEST.out;
...
halt() {
   ...
   for chrome in $(pgrep chromedriver)
   do
     echo "Killing stale chromedriver instance $chrome"
     recursive_kill -9 $chrome ">"
   done
   ...
   echo "[debug] Still open processes:"
   ps

   echo "Writing report.html"
   rebot --outputdir target --report report.html --name Roboto target/*.xml
   CHROME_COUNT=$(pgrep chromedriver | wc -l)
   echo "Trying to kill again $CHROME_COUNT chromedrivers..."
   for chrome in $(pgrep chromedriver)
   do
     recursive_kill -9 $chrome ">"
   done
...
# On terminate/interrupt run halt
trap halt SIGTERM SIGINT
