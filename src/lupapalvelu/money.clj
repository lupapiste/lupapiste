...
(defn ->formatted-string [money-value]
  (mf/format money-value locale))

(sc/defn ->MoneyResponse [money-value]
  {:major (ma/major-of money-value)
   :minor (ma/minor-of money-value)
   :text (->formatted-string money-value)
   :currency (.toString (ma/currency-of money-value))})

(defn MoneyResponse->Money [{:keys [minor currency]}]
  (minor->currency (mc/for-code currency) minor))

(def ->currency-text (comp ->formatted-string ->currency))
...
(defn vat-value
  "Multiplies money with given multiplier. Multipier needs to be positive."
  [^Money money ^double vat-multiplier]
  {:pre [(>= vat-multiplier 1.0)]}
  (ma/multiply money vat-multiplier :half-up))
