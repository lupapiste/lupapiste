...
(defn- pate-enabled
  ...
  (when (some-> command :data :organizationId)
    (if-let [organization (template/command->organization command)]
      (when-not (some #(get-in % [:pate :enabled]) (:scope organization))
...
(defcommand new-verdict-template
  ...
  and draft."
   ...
   :input-validators [(partial action/non-blank-parameters [:organizationId :category])]
   :pre-checks       [pate-enabled
                      valid-category]}
  [command]
...
(defcommand set-verdict-template-name
  {:description      "Name cannot be empty."
   :parameters       [:organizationId :template-id :name]
   :input-validators [(partial action/non-blank-parameters [:organizationId :template-id :name])]
   :pre-checks       [pate-enabled
...
(defcommand save-verdict-template-draft-value
  ...
  drafts. Returns modified timestamp."
   :permissions      [{:required [:organization/admin]}]
   :parameters       [:organizationId :template-id :path :value]
   ;; Value is validated upon saving according to the schema.
   :input-validators [(partial action/vector-parameters [:path])
                      (partial action/non-blank-parameters [:template-id :organizationId])]
   :pre-checks       [pate-enabled
                      (template/verdict-template-check :editable)]}
...
(defcommand publish-verdict-template
  ...
  template draft. The snapshot includes also the current settings."
   :permissions      [{:required [:organization/admin]}]
   :parameters       [:organizationId :template-id]
   :input-validators [(partial action/non-blank-parameters [:organizationId :template-id])]
   :pre-checks       [pate-enabled
...
(defcommand toggle-delete-verdict-template
  {:description      "Toggle template's deletion status"
   :permissions      [{:required [:organization/admin]}]
   :parameters       [:organizationId :template-id :delete]
   :input-validators [(partial action/non-blank-parameters [:organizationId :template-id])
...
(defcommand save-verdict-template-settings-value
  ...
  settings. Returns modified timestamp. Creates settings if needed."
   :permissions      [{:required [:organization/admin]}]
   :parameters       [:organizationId :category :path :value]
...
(defcommand set-default-operation-verdict-template
  ...
  cleared."
   ...
   :input-validators    [(partial action/non-blank-parameters [:organizationId :operation])]
   :pre-checks          [pate-enabled
                         (template/verdict-template-check :published :editable :blank)
                         organization-operation
                         operation-vs-template-category]}
  [_]
  (template/set-operation-verdict-template organizationId operation template-id))
