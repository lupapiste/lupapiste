(ns lupapalvelu.exports
  ...
  (:require [lupapalvelu.application-utils :as app-utils]
...
(def rakennusluokka-hinnasto (delay (xls/read-map "rakennusluokka_hinnasto.xlsx")))
...
(defn- uuden-rakentaminen [application operation]
  ...
  (let [doc (domain/get-document-by-operation application operation)
        ...
        [_ rakennusluokka]  (first (tools/deep-find doc [:rakennusluokka :value]))
        rakennusluokka (when (and (ss/not-blank? rakennusluokka)
                                  (contains? (tools/resolve-schema-flags {:organization (org/get-application-organization application)
                                                                          :application application})
                                             :rakennusluokka))
                         rakennusluokka)]
    (if rakennusluokka
      {:price-class (get @rakennusluokka-hinnasto rakennusluokka) :rakennusluokka rakennusluokka}
...
(def aiemmalla-luvalla-hakeminen-price-code 914)

(def price-classes-for-operation
  "Legacy price-classes already superceded by the price-code,
  especially irrelevant once Lupapiste moves away from transaction fees to subscription fees entirely"
  {:asuinrakennus               uuden-rakentaminen ; old operation tree
   ...
   :vesiliikennelaki-kilpailut-ja-harjoitukset                        "F"
   :archiving-project                                                 "Z"
   :lyhytaikainen-maanvuokraus "D"
   :promootio                  "D"
   :konversio                  "F"
   :rasite-tai-yhteisjarjestely "F" ; F = free, just in case someone still runs the salesforce export
   :ymparistoluvan-selventaminen "F"
   :yleinen-ilmoitus             "F"
   :talousjatevesien-kasittelysta-poikkeaminen "F"
   :maa-aineslupa-jatkoaika "F"
   :ylijaamamaiden-hyodyntaminen "F"
   :leirintaalueilmoitus "F"
   :jatteiden-hyodyntaminen-maarakentamisessa "F"
   :rekisterointi-ilmoitus "F"
   :yhteiskasittely-maa-aines-ja-ymparistoluvalle "F"
   :lannan-levittamisesta-poikkeustilanteessa "F"
   :kirjallinen-vireillepano "F"
   :lannan-varastointi-v2 "F"
   })
...
(defn- resolve-price-class
  ...
  (let [op-name  (keyword (:name op))
    ...
    (cond
      ...
      ;; Same price for both frame agreement (puitesopimus) and normal customers
      (assoc op
        :priceClass price-class
        :priceCode aiemmalla-luvalla-hakeminen-price-code
        :use nil
        :useFi nil
        :useSv nil
        :usagePriceCode aiemmalla-luvalla-hakeminen-price-code)

      ; new buildings: fixed price code, but usage price code is determined from the building data
      (= uuden-rakentaminen price-class)
      (let [{:keys [price-class kayttotarkoitus rakennusluokka]} (uuden-rakentaminen application op)]
        (assoc op
          :priceClass price-class
          :priceCode 900
          :use (if (ss/not-blank? rakennusluokka)
                 rakennusluokka
                 kayttotarkoitus)
          :useFi (if (ss/not-blank? rakennusluokka)
                   (i18n/localize :fi :rakennusluokka rakennusluokka)
                   (when-not (ss/blank? kayttotarkoitus) (i18n/localize :fi :kayttotarkoitus kayttotarkoitus)))
          :useSv (if (ss/not-blank? rakennusluokka)
                   (i18n/localize :sv :rakennusluokka rakennusluokka)
                   (when-not (ss/blank? kayttotarkoitus) (i18n/localize :sv :kayttotarkoitus kayttotarkoitus)))
...
(defn application-to-salesforce [application]
  (letfn [(truncate-op-description [op] (update op :description #(ss/limit % 252 "...")))
          (map-operations [app] (->> (app-utils/get-operations app)
...
(sc/defschema SalesforceExportApplication
  ...
  (merge {:id                                ssc/ApplicationId
          :address                           sc/Str
          :archived                          archiving-util/archived-ts-keys-schema
          :infoRequest                       sc/Bool
          :municipality                      sc/Str
