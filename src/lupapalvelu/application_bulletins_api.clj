...
(def- sort-field-mapping {"bulletinState" :versions.bulletinState
                          ...
                          "verdictGivenAt" :versions.verdictGivenAt})

(def- default-sort (array-map :versions.modified -1))
...
(defquery local-application-bulletins
  {:parameters [searchText organization sort]
...
(defn- bulletin-can-be-commented
  ([{{bulletin-id :bulletinId} :data}]
   (let [projection {:bulletinState 1 "versions.proclamationStartsAt" 1 "versions.proclamationEndsAt" 1}
...
(defn- referenced-file-can-be-attached
  [{{files :files} :data}]
  (when-not (->> files
                 (map #(storage/unlinked-file-exists? (vetuma/session-id) (:fileId %))))
...
(defcommand move-to-verdict-given
  {:parameters       [id verdictGivenAt appealPeriodStartsAt appealPeriodEndsAt verdictGivenText]
   :input-validators [(partial action/non-blank-parameters [:id :verdictGivenText])
                      ...
                      (partial bulletins/validate-input-dates :verdictGivenAt :appealPeriodStartsAt)]
   :user-roles       #{:authority}
   :states           #{:verdictGiven}
   :pre-checks       [(permit/validate-permit-type-is permit/YI permit/YL permit/YM permit/VVVL  permit/MAL)
...
(defquery bulletin
  {:description      "Returns the current/latest application bulletin version."
  ...
  (if-let [bulletin (bulletins/get-bulletin bulletinId)]
    (let [latest-version       (bulletins/bulletin-version-for-user command bulletin)
      ...
      (bulletins/add-version-visit bulletinId (:id latest-version) created)
...
(defn- bulletin-can-be-saved
  ([state {{bulletin-id :bulletinId} :data}]
   (let [bulletin (bulletins/get-bulletin bulletin-id)]
     (if-not bulletin
       (fail :error.invalid-bulletin-id)
       (when-not (= (:bulletinState bulletin) state)
         (fail :error.invalid-bulletin-state)))))
  ([state command _]
...
(defquery ymp-publish-bulletin-enabled
  {:description "Bulletin implementation for YMP permit types enabled"
   ...
   :user-roles #{:authority :applicant}
   :pre-checks [(permit/validate-permit-type-is permit/YI permit/YL permit/YM permit/VVVL  permit/MAL)
                (action/not-pre-check (op/validate-primary-operation-is :ymparistoluvan-selventaminen))]})
