...
(defn update-guest-authority-organization
  ...
  [organizationId org-admin email first-name last-name description]
  (let [email (ss/canonize-email email)
        guests (->> organizationId
                    organization-guest-authorities
                    (remove #(= email (:email %)))
                    (concat [{:email       email
                              :name        (str first-name " " last-name)
                              :description description}]))]
    (org/update-organization organizationId {$set {:guestAuthorities guests}})
    (when-not (usr/get-user-by-email email)
      (let [new-user-data {:firstName first-name
        ...
        (uu/notify-new-authority user org-admin organizationId)))))
...
(defn remove-guest-authority-organization
  ...
  (let [email (ss/canonize-email email)
    ...
    (org/update-organization organizationId {$pull {:guestAuthorities {:email email}}})
    ...
    (when guest-id
      (mongo/update-by-query :applications
                             {:organization organizationId :auth {$elemMatch match}}
                             {$pull {:auth match}}))))
