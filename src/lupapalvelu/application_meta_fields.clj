...
(defn- statements-summary [{user-id :id} {:keys [statements] :as app}]
  (when (and (seq statements)
             (auth/has-auth-role? app user-id :statementGiver))
    (->> statements
         (filter #(= user-id (get-in % [:person :userId])))
         (reduce
           (fn [result {:keys [state id]}]
             (condp contains? (keyword state)
               statement-schemas/pre-given-states (update result :open conj id)
               statement-schemas/post-given-states (update result :given conj id)))
           {:open []
            :given []}))))
...
(defn- organization-meta [_ app]
  (let [org (organization/get-organization (:organization app))
    ...
    {:name (organization/get-organization-name org)
     ...
     :planInfoDisabled (:plan-info-disabled org)
...
(def indicator-meta-fields [{:field :documentModificationsPerDoc :fn count-document-modifications-per-doc}
                            ...
                            {:field :statementsSummary :fn statements-summary}])
