...
(defn initial-state [graph]
  ...
  (cond
    ...
    (instance? PersistentArrayMap graph) (first (keys graph))
...
(def rakennusrasite-application-state-graph common-states/rakennusrasite-application-state-graph)

(def selventaminen-application-state-graph common-states/selventaminen-application-state-graph)
...
(def all-but-terminal (difference all-states terminal-states))
...
(def archival-final-states (-> terminal-states
                               (disj :canceled :answered :registered)
