...
(defn valid-font-resource? [font-resource]
  "Font resource must be Java [Resource or URL] path to [Type1 or TrueType] font file."
  (let [path (clojure.string/lower-case font-resource)]
    (when-not (or
                (.endsWith path ".afm")
                (.endsWith path ".pfm")
                (.endsWith path ".ttf")
                (.endsWith path ".otf")
                (.endsWith path ".ttc"))
      (throw (IllegalArgumentException. (str "Illegal Font-file extension: " path))))
    (when-not (> (with-open [xin (io/input-stream (io/resource path))] (.available xin)) 0)
      (throw (IllegalArgumentException. (str "Font-file '" font-resource "' not found or is empty"))))))
...
(defn font
  ...
  (let [resource-url (if (empty? ttf-name)
        ...
        font-style (cond
                     ...
                     :else Font/NORMAL)
        font-colour (if (and r g b) (new CMYKColor (int r) (int g) (int b) (min (int r) (int g) (int b))) (new Color 0 0 0 1))]
    ; for UnicodeTrueType the encoding needs to be BaseFont/IDENTITY_H or BaseFont/IDENTITY_V
    (FontFactory/getFont resource-url BaseFont/WINANSI true font-size font-style font-colour)))
...
(defn- setup-doc [{:keys [left-margin
  ...
  (let [[nom head] doc-header
    ...
    (let [output-stream-to-use (if (or pages (not (empty? page-events))) temp-stream output-stream)
      ...
      (.setPDFXConformance pdf-writer PdfWriter/PDFA1B)
      (when-not pages
      ...
      ;(.put (.getExtraCatalog pdf-writer) PdfName/LANG (PdfString. "sv-fi"))
      ...
      (.addKeywords doc "Lupapiste.fi, XMP, Metadata")

      (.createXmpMetadata pdf-writer)
