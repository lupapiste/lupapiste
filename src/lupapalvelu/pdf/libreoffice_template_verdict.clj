...
(defn- verdict-dates [lang dates]
  (cond-> []
          (:anto dates) (conj [(i18n/localize lang "verdict.anto") (or (fi-date (:anto dates)) "-")])
          (and (:lainvoimainen dates) (> (:lainvoimainen dates) 0)) (conj [(i18n/localize lang "verdict.lainvoimainen") (or (fi-date (:lainvoimainen dates)) "-")])
          (:aloitettava dates) (conj [(i18n/localize lang "verdict.aloitettava") (or (fi-date (:aloitettava dates)) "-")])
          (:voimassaHetki dates) (conj [(i18n/localize lang "verdict.voimassaHetki") (or (fi-date (:voimassaHetki dates)) "-")])
          (:viimeinenValitus dates) (conj [(i18n/localize lang "verdict.viimeinenValitus") (or (fi-date (:viimeinenValitus dates)) "-")])
          (:raukeamis dates) (conj [(i18n/localize lang "verdict.raukeamis") (or (fi-date (:raukeamis dates)) "-")])
          (:paatosdokumentinPvm dates) (conj [(i18n/localize lang "verdict.paatosdokumentinPvm") (or (fi-date (:paatosdokumentinPvm dates)) "-")])
          (:julkipano dates) (conj [(i18n/localize lang "verdict.julkipano") (or (fi-date (:julkipano dates)) "-")])
          (:paatosPvm dates) (conj [(i18n/localize lang "verdict.paatosPvm") (or (fi-date (:paatosPvm dates)) "-")])))
...
(defn- verdict-signatures [verdict paatos]
  (into [[(verdict-paatos-key paatos :paatoksentekija) (or (fi-date (get-in paatos [:paivamaarat :anto])) "-")]]
        (map (fn [sig] [(str (get-in sig [:user :firstName]) " " (get-in sig [:user :lastName])) (or (fi-date (:created sig)) "-")]) (:signatures verdict))))

(defn- get-vastuuhenkilo [application]
    (let [data (domain/get-document-by-subtype application :tyomaasta-vastaava)
...
(defn write-verdict-libre-doc [application id paatos-idx lang file]
  (let [applicants (map (fn [val] [val]) (map template/name-from-doc (template/get-applicant-docs application)))
        ...
        start-time (fi-date (template/get-in-document-data application :tyoaika [:tyoaika-alkaa-ms :value]))
        end-time (fi-date (template/get-in-document-data application :tyoaika [:tyoaika-paattyy-ms :value]))]
    ...
    (template/create-libre-doc (io/resource (if (:sopimus verdict) "private/lupapiste-ya-contract-template.fodt" "private/lupapiste-ya-verdict-template.fodt"))
                               ...
                               (assoc (template/common-field-map application lang)
                                 ...
                                 "LPVALUE_VERDICT_GIVEN" (or (fi-date (get-in paatos [:paivamaarat :anto])) "-")
                                 ...
                                 "LPVALUE_VERDICT_LEGAL" (or (if (> (get-in paatos [:paivamaarat :lainvoimainen]) 0) (fi-date (get-in paatos [:paivamaarat :lainvoimainen]))) "-")
