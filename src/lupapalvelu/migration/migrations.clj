  ...
  (let [application (domain/get-application-no-access-checking lp-id)
        ...
        [op-name doc-names] (when application-does-not-contain-rakennuspaikka? ;; get the operation that needs the rakennuspaikka document
                              (->> (cons (:primaryOperation application) (:secondaryOperations application))
                                   ...
                                   (sort-by (fn [[k _]] k))
                                   (first)))
        rakennuspaikka-schema (when op-name
                                (->> doc-names
                                     (map #{"rakennuspaikka" "rakennuspaikka-ilman-ilmoitusta"})
                                     (remove nil?)
                                     (first)
                                     (keyword)
...
(defmigration fix-broken-applications-that-replaced-operation
  (let [broken-apps ["LP-536-2018-00148" "LP-837-2018-00915" "LP-423-2018-00130" "LP-837-2017-03424" "LP-753-2017-00828"
