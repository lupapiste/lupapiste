(ns lupapalvelu.company-api
  (:require [lupapalvelu.action :refer [defquery defcommand some-pre-check notify] :as action]
...
(defquery company
  {:user-roles #{:applicant :authority}
   ...
   :parameters [company]}
  [{{:keys [users]} :data}]
  (ok :company     (dissoc (com/find-company! {:id company})
                           :submitRestrictor)
  ...
  {:parameters       [tags]
   :input-validators [(partial action/parameters-matching-schema
                               ...
                               [{(sc/optional-key :id) (sc/maybe ssc/ObjectIdStr)
                                 :label                ssc/NonBlankStr}])]
   :user-roles       #{:applicant}
   :pre-checks       [(com/validate-has-company-role :any)]}
  [{{:keys [company] :as user} :user}]
  (com/update-company! (:id company) {:tags (map mongo/ensure-id (ss/trimwalk tags))} user)
  (ok))
