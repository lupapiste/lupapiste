...
(defschema Huoneisto
  {(sc/optional-key :huoneistonumero)         sc/Str
   ...
   (sc/optional-key :pysyvaHuoneistotunnus)   sc/Str})
...
(defschema WFS-building
  {(sc/optional-key :muutostyolaji)                      sc/Any ;;TODO: Find out correct type
   ...
   (sc/optional-key :kaytto)                             {(sc/optional-key :kayttotarkoitus)              sc/Str  ;;TODO: Enumeration or just String?
                                                          ...
                                                          (sc/optional-key :rakennusluokka)               sc/Str}
...
(defschema OperationName
  (apply sc/enum (map name (keys operations/operations))))
...
(defschema Operation
  {:id                            ssc/ObjectIdStr
   ...
   (sc/optional-key :extinct)     (sc/maybe ssc/Timestamp)})
...
(defschema Document-building
  {:id       ssc/ObjectIdStr
   ...
   :created  ssc/Timestamp
   (sc/optional-key :secret) sc/Bool})

(defschema State
  (apply sc/enum (map name states/all-states)))

(defschema PermitType
  (apply sc/enum (map name (keys (permit/permit-types)))))
...
(defschema Application                                      ; WIP, used initially in state-change JSON
  {:id                                             ssc/ApplicationId
   ...
   :infoRequest                                    sc/Bool
   (sc/optional-key :non-billable-application)     sc/Bool
   (sc/optional-key :_skip-task-validation)        sc/Bool
   (sc/optional-key :3dMapActivated)               (sc/maybe ssc/Timestamp)
...
(defschema ForemanTermination
  "Termination field for foreman applications.
   Nil if not a foreman application or not terminated."
  {(sc/optional-key :started)    ssc/Timestamp
   (sc/optional-key :ended)      ssc/Timestamp
   :reason                       sc/Str
   :state                        (sc/enum "requested" "confirmed")
   :requester                    usr/SummaryUser
   (sc/optional-key :request-ts) ssc/Timestamp
   (sc/optional-key :confirmer)  usr/SummaryUser})
