        ...
        (let [new-app (new-application-copy source-app user organization created {})
          ...
          (fact "the application is copied almost verbatim"
            (let [[only-new _ _] (diff (dissoc-ids-and-timestamps new-app)
              ...
              => (just [:auth :attachments :comments :history :creator :_creatorIndex]
                       :in-any-order))
            (keys new)        ; gives the same result as (keys old)
            => (just [:auth :attachments :comments :created :documents :history
                      :id :modified :primaryOperation :creator :_creatorIndex]
                     :in-any-order))
          ...
          (fact "copier is the creator"
            (:_creatorIndex new-app) => "New User"
...
(facts "document intricacies"
  ...
  (facts "clear-personal-information"
    (let [application {:created 12345
      ...
      (fact "if personal details are inputed by hand (no userId), they are copied as is" ; TT-18617
        (-> (assoc-in hakija-document [:henkilo :userId :value] "")
            (clear-personal-information application)
            :henkilo)
        =>
        (-> hakija-document
...
(fact "non-copyable-auth"
  (let [user            {:id      "user"
                         :company {:id "company"}}
        statement-giver {:id "sg" :role "statementGiver"}
        foreman-invite  {:id     "tj"
                         :invite {:role "tyonjohtaja"}}
        guest           {:id "g" :role "guest"}
        guest-authority {:id "ga" :role "guestAuthority"}
        company         {:id "company"}]
    (non-copyable-auth? user user) => true
    (non-copyable-auth? statement-giver user) => true
    (non-copyable-auth? foreman-invite user) => true
    (non-copyable-auth? guest user) => true
    (non-copyable-auth? guest-authority user) => true
    (non-copyable-auth? company user) => true))
