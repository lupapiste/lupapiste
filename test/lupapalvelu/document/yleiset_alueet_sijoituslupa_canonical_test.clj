...
;        hakija-yksityinen-osoite (:osoite maksaja-yksityinen-Henkilo) => truthy
    ...
    (fact "Kasittelytieto-muutosHetki" (:muutosHetki Kasittelytieto) => (date/xml-datetime (:modified sijoituslupa-application)))
    ...
    (fact "Kasittelytieto-paivaysPvm" (:paivaysPvm Kasittelytieto) => (date/xml-date (:opened sijoituslupa-application)))
    ...
    (facts "maksaja-yksityinen-ulkomainen-hetu"
      (let [get-maksaja #(-> % tools/unwrapped
                             get-hakija
                             :Osapuoli :henkilotieto :Henkilo)]
        (fact "maksaja-yksityinen-using-ulkomainen-hetu"
          (select-keys (get-maksaja hakija-using-ulkomainen-hetu) [:ulkomainenHenkilotunnus :henkilotunnus])
          => {:ulkomainenHenkilotunnus "12345"})
        (fact "maksaja-yksityinen-using-finnish-hetu"
          (select-keys (get-maksaja hakija-using-finnish-hetu) [:ulkomainenHenkilotunnus :henkilotunnus])
          => {:henkilotunnus "010203-040A"})))
...
(def sijoituslupa-application-with-extras
  (assoc sijoituslupa-application :documents [hakija
                                              hakija-using-ulkomainen-hetu
                                              maksaja
                                              hankkeen-kuvaus-sijoituslupa
                                              (assoc-in tyomaasta-vastaava [:schema-info :name] "tyomaasta-vastaava-optional")
                                              asiamies]))

(ctc/validate-all-documents sijoituslupa-application-with-extras)

(defn match-fn [m]
  (fn [candidate]
    (every? (fn [[k v]]
              (= (get-in candidate (util/split-kw-path k)) v))
            m)))

(defn find-matches [m targets]
  (filter (match-fn m) targets))

(facts "Cannical with multiple applicants, a site responsible person and an agent"
  (let [parties (-> (application-to-canonical sijoituslupa-application-with-extras "fi")
                    :YleisetAlueet :yleinenAlueAsiatieto :Sijoituslupa :osapuolitieto)]
    (fact "Two applicants"
      (find-matches {:Osapuoli.rooliKoodi "hakija"} parties) => (just anything anything)
      (find-matches {:Osapuoli.rooliKoodi                  "hakija"
                     :Osapuoli.henkilotieto.Henkilo.osoite nil
                     :Osapuoli.yritystieto.Yritys.nimi     "Yritys Oy Ab"}
                    parties) => (just anything)
      (find-matches {:Osapuoli.rooliKoodi                                    "hakija"
                     :Osapuoli.henkilotieto.Henkilo.osoite.osoitenimi.teksti "Paapankuja 12"
                     :Osapuoli.henkilotieto.Henkilo.henkilotunnus            nil
                     :Osapuoli.henkilotieto.Henkilo.ulkomainenHenkilotunnus  "12345"
                     :Osapuoli.yritystieto.Yritys.nimi                       nil}
                    parties) => (just anything))
    (fact "Agent"
      (find-matches {:Osapuoli.rooliKoodi "yhteyshenkilö"} parties) => (just anything)
      (find-matches {:Osapuoli.rooliKoodi                                    "yhteyshenkilö"
                     :Osapuoli.henkilotieto.Henkilo.osoite.osoitenimi.teksti "Joku katu"
                     :Osapuoli.henkilotieto.Henkilo.nimi                     {:etunimi "Aino" :sukunimi "Asiamies"}
                     :Osapuoli.yritystieto.Yritys.nimi                       nil}
                    parties) => (just anything))
    (fact "Optional site responsible person"
      (find-matches {:Osapuoli.rooliKoodi                                "työnsuorittaja"
                     :Osapuoli.henkilotieto.Henkilo.osoite               nil
                     :Osapuoli.yritystieto.Yritys.nimi                   "Yritys Oy Ab"
                     :Vastuuhenkilo.etunimi                              "Pena"
                     :Vastuuhenkilo.osoitetieto.osoite.osoitenimi.teksti "Paapankuja 12"
                     :Vastuuhenkilo.rooliKoodi                           "lupaehdoista/työmaasta vastaava henkilö"}
                    parties) => (just anything))))
