...
(facts create-doc-validator
  (facts "pientalo - can always add more"
    (let [command {:application {:permitType "R"
                                 :primaryOperation {:name "pientalo"}
                                 :state "submitted"
                                 :schema-version 1
                                 :documents  (mapv (fn [schema-name] {:schema-info {:type :party, :name schema-name}})
                                                   (concat [@#'op/applicant-doc-schema-name-R]
                                                           @#'op/common-rakval-schemas
                                                           @#'op/optional-rakval-schemas))}
                   :data {:schemaName "hakija-r"}}]
      (create-doc-validator command) => nil
      (create-doc-validator (keywordize-schema-info-types command)) => nil
      (fact "For allowed-actions"
        (create-doc-validator (dissoc command :data)) => nil)))

  (facts "sijoituslupa"
    (let [application {:permitType "YA"
                       :primaryOperation {:name "ya-sijoituslupa-vesi-ja-viemarijohtojen-sijoittaminen"}
                       :state "submitted"
                       :schema-version 1
                       :documents  (mapv (fn [schema-name] {:schema-info {:type :party, :name schema-name}})
                                         (cons @#'op/applicant-doc-schema-name-YA
                                               @#'op/common-yleiset-alueet-schemas))}
          make-cmd (fn [schema-name]
                     {:application application
                      :data {:schemaName schema-name}})]
      (fact "Applicant can be added"
        (create-doc-validator (make-cmd "hakija-ya")) => nil
        (create-doc-validator (keywordize-schema-info-types (make-cmd "hakija-ya"))) => nil)
      (fact "Agent can be added"
        (create-doc-validator (make-cmd "hakijan-asiamies")) => nil
        (create-doc-validator (keywordize-schema-info-types (make-cmd "hakijan-asiamies"))) => nil)
      (fact "Optional site responsible person can be added"
        (create-doc-validator (make-cmd "tyomaasta-vastaava-optional")) => nil
        (create-doc-validator (keywordize-schema-info-types (make-cmd "tyomaasta-vastaava-optional"))) => nil)))

  (facts "promootio"
    (let [application {:permitType       "A"
                       :primaryOperation {:name "promootio"}
                       :state            "submitted"
                       :schema-version   1
                       :documents        (mapv (fn [schema-name] {:schema-info {:type :party, :name schema-name}})
                                               (cons @#'op/applicant-doc-schema-name-YA
                                                     @#'op/common-yleiset-alueet-schemas))}]
      (facts "can add asiamies"
        (let [command {:application application
                       :data        {:schemaName "hakijan-asiamies"}}]
          (create-doc-validator command) => nil
          (create-doc-validator (keywordize-schema-info-types command)) => nil))

      (facts "can't add another asiamies"
        (let [command {:application (update application :documents conj
                                            {:schema-info {:type :party, :name "hakijan-asiamies"}})
                       :data        {:schemaName "hakijan-asiamies"}}]
          (create-doc-validator command) => {:ok false, :text "error.create-doc-not-allowed"}
          (create-doc-validator (keywordize-schema-info-types command))
          => {:ok false, :text "error.create-doc-not-allowed"}))

      (facts "can't add hakija"
        (let [command {:application application
                       :data        {:schemaName "hakija-ya"}}]
          (create-doc-validator command) => {:ok false, :text "error.create-doc-not-allowed"}
          (create-doc-validator (keywordize-schema-info-types command))
          => {:ok false, :text "error.create-doc-not-allowed"})))))
...
(facts "document context function"
  (let [guest       {:id "1" :role "guest"}
    ...
    (fact "foreman receives edit permissions for hakija-r"
