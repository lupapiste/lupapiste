...
(facts "document-raki-conversion"
  ...
  (fact "no building, no change"
    (document-raki-conversion ..anything.. (documents 1)) => (documents 1)
    (document-raki-conversion ..anything.. (documents 2)) => (documents 2))

  (fact "national building id is generated"
    (document-raki-conversion "09100300520010" (documents 0)) =>
...
(facts "building-raki-conversion"
  ...
  (building-raki-conversion "09104001530009"
    {:localShortId "016"
     :buildingId "016"
     :index "1"
     :created "2015"
...
(facts "task-raki-conversion"
  ...
  (fact "no building, no change"
    (task-raki-conversion ..anything.. (tasks 0)) => (tasks 0)
    (task-raki-conversion ..anything.. (tasks 2)) => (tasks 2)
    (task-raki-conversion ..anything.. (tasks 3)) => (tasks 3)
    (task-raki-conversion ..anything.. (tasks 4)) => (tasks 4))

  (fact "national building id is generated"
    (task-raki-conversion "09100200400009" (tasks 1)) =>
    {:data {:katselmuksenLaji {:value "aloituskokous"} :vaadittuLupaehtona {:value true}
            :rakennus {:1 {}
