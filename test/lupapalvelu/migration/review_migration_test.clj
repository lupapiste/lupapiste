...
(def attachments [{:id "att1"}
                  ...
                  {:id     "att12"
                   ...
                   :target {:id   "old-six"
                            :type "task"}}])
(def t1          (make-task "one" "review-backend" "One" "" 2000))
(def t1-old      (make-task "old-one" "review-backend" "One" "LP-foo-bar" 1500))
(def t2          (make-task "two" "review-backend" "Two" "" 2200))
(def t2-old      (make-task "old-two" "review-backend" "Two" "LP-foo-bar" 1700))
(def t3          (make-task "three" "review-backend" "Three" "not-empty" 3000))
(def t3-old      (make-task "old-three" "review-backend" "Three" "not-empty" 3000))
(def t4          (make-task "four" "wrong" "Four" "" 4000))
(def t4-old      (make-task "old-four" "wrong" "Four" "" 1600))
(def t5          (make-task "five" "review-backend" "Five" "not-empty" 5000))
(def t5-old      (make-task "old-five" "review-backend" "Five" "" 2000))
(def t6          (make-task "six" "review-backend" "Six" "" 6000))
(def t6-old      (make-task "old-six" "review-backend" "Six" "" 5000))
(def t6-older    (make-task "older-six" "review-backend" "Six" "LP-foo-bar" 4000))
(def t7          (make-task "seven" "review-backend" "Seven" "" 7000))
(def application {:tasks       (shuffle [t1 t1-old t2 t2-old t3 t3-old
                                         t4 t4-old t5 t5-old
