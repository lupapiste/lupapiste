  ...
  (doseq [lang test-util/test-languages]
    (files/with-temp-file tmp-file
      ...
      (let [res (s/split (slurp tmp-file) #"\r?\n")
        ...
        (fact {:midje/description (str " Statement libre document teksti (" (name lang) ")")} (nth res (+ doc-start-row 81)) => #(s/includes? % "Lausunto liitteen"))
        (fact {:midje/description (str " Statement libre document status (" (name lang) ")")} (nth res (+ doc-start-row 87)) => #(s/includes? % "ehdoilla"))
        (fact {:midje/description (str " Statement libre document attachments (" (name lang) ")")} (nth res (+ doc-start-row 91)) => #(s/includes? % (str "1 " (i18n/localize lang "unit.kpl"))))))))
