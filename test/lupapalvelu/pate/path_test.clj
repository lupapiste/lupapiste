...
(fact "path-truthy?"
  (let [options {:state (atom {:foo "hello"
    ...
    (fact "computed"
      (let [options (update options :state swap! assoc :verdict-code "evatty")]
        (path-truthy? options :_computed.negative-verdict?) => true
        (path-truthy? options :_computed.positive-verdict?) => false))))
